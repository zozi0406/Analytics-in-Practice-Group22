...
tc_normal<-trainControl(method = "cv" ,number = 5 ,allowParallel = T,classProbs = TRUE,summaryFunction = prSummary, savePredictions = TRUE)
tc_smote<-trainControl(method = "cv" ,number = 5 ,allowParallel = T,classProbs = TRUE,summaryFunction = prSummary,sampling="smote", savePredictions = TRUE)
tc_over<-trainControl(method = "cv" ,number = 5 ,allowParallel = T,classProbs = TRUE,summaryFunction = prSummary,sampling="up", savePredictions = TRUE)
tc_under<-trainControl(method = "cv" ,number = 5 ,allowParallel = T,classProbs = TRUE,summaryFunction = prSummary,sampling="down", savePredictions = TRUE)
...
for (i in names(baseline)) {
    baseline_res<-baseline[[i]]$results %>% add_column(dataset=i) %>%  bind_rows(baseline_res)
}
...
model_comparison$gbm<-evaluate_model(data=training_set, method="gbm",train_control = train_control)
model_comparison$rf<-evaluate_model(data=training_set, method="rf",train_control = train_control)
model_comparison$svm_lin<-evaluate_model(data=training_set, method="svmLinear",train_control = train_control)
model_comparison$svm<-evaluate_model(data=training_set, method="svmRadial",train_control = train_control)
model_comparison$xgb<-evaluate_model(data=training_set, method="xgbTree",train_control = train_control)
model_comparison$tree<-evaluate_model(data=training_set, method="C5.0",train_control = train_control)
model_comparison$nnet<-evaluate_model(data=training_set, method="nnet",train_control = train_control)

model_res <- data.frame()
for (i in names(model_comparison)) {
    model_res<-model_comparison[[i]]$results %>% add_column(dataset=i) %>%  bind_rows(model_res)
}
...
baseline_eval<-evalm(baseline,positive="DEFAULT",optimise = "F1",showplots=F)
