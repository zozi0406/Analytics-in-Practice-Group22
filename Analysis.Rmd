...
```{r baseline_modeling_try}
set.seed(100)

tc<-trainControl(method = "cv" ,number = 5 ,allowParallel = T,classProbs = TRUE,summaryFunction = twoClassSummary)

evaluate_model <-
function(formula=class~.,data,method,train_control=tc,tuneLength=1, grid=NULL, ...) {
...
# rich_smote
test_rich_smote <- evaluate_model(data=train_sets$rich_smote, method="gbm",verbose=T,tuneLength=1,grid=tunegrid)

test_rich_smote$fit
test_rich_smote$cm

### TEST EVALUATION FOR THE FINAL STEP
(tst<-testeval(test_rich,test_rich_smote$fit))
...
```{r}
# sparse_smote
test<-evaluate_model(data=train_sets$sparse_smote, method="gbm",verbose=T,tuneLength=1,grid=tunegrid)
