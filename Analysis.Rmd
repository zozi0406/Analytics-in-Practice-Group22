...
rich_smote_test<-evaluate_model(data=train_sets$rich_smote, method="gbm",verbose=T,tuneLength=1,grid=tunegrid)

rich_smote_test$fit
rich_smote_test$cm

### TEST EVALUATION FOR THE FINAL STEP
(sparse_smote_tst<-testeval(test_rich,rich_smote_test$fit))
...
# feature plot
#x = select(test_rich,PY1:PY6)
#y= test_rich$class
#featurePlot(x, y, "box", jitter = TRUE)

# Create Data to Plot a Learning Curve
set.seed(111)
chart_data <- as.data.frame(train_sets$rich_smote)
lc <-
  learning_curve_dat(chart_data,
                     outcome = "class",
                     method = "gbm",
                     metric = "ROC",
                     trControl = tc)
...
ggplot(lc, aes(ROC,Training_Size),color = Data) +
  geom_smooth(method = loess, span = .8) +
  theme_bw()



# densityplot:"Score distribution for binary model"
pl1 <- densityplot(sparse_smote_test$fit,alpha = 0.6,adjust = 1.25)
pl2 <- densityplot(rich_smote_test$fit,alpha = 0.6,adjust = 1.25)
pl3 <- densityplot(rich_test$fit,alpha = 0.6,adjust = 1.25)
pl3 <- densityplot(sparse_test$fit,alpha = 0.6,adjust = 1.25)
# 

dotPlot(varImp(test$fit))

chart.fit <- gbm(class~., data = rich_smote_test$fit, n.trees = 250, interaction.depth = 3)
chart.exp <- explain(chart.fit,
                        data = train_sets$rich_smote,
                        y = train_sets$rich_smote$class,
...
```{r}
# sparse_smote
sparse_smote_test<-evaluate_model(data=train_sets$sparse_smote, method="gbm",verbose=T,tuneLength=1,grid=tunegrid)

sparse_smote_test$fit
sparse_smote_test$cm

### TEST EVALUATION FOR THE FINAL STEP
(sparse_smote_tst<-testeval(test_sparse,sparse_smote_test$fit))
```

```{r}
# sparse
test<-evaluate_model(data=train_sets$sparse, method="gbm",verbose=T,tuneLength=1,grid=tunegrid)

test$fit
test$cm

### TEST EVALUATION FOR THE FINAL STEP
(tst<-testeval(test_sparse,test$fit))
```

```{r}
# rich_smote
test<-evaluate_model(data=train_sets$rich, method="gbm",verbose=T,tuneLength=1,grid=tunegrid)
